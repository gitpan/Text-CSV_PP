...
{   # http://rt.cpan.org/Ticket/Display.html?id=24386
    ...
    ok (my $csv = Text::CSV_PP->new ({ sep_char => "\t" }), "RT-24386: \\t doesn't work");
...
{   # http://rt.cpan.org/Ticket/Display.html?id=21530
    ...
    close FH;
    ok (my $csv = Text::CSV_PP->new ({ binary => 1 }), "RT-21530: getline () return at eof");
    open  FH, "<_test.csv";
    my $row;
    foreach my $line (1 .. 5) {
...
{   # http://rt.cpan.org/Ticket/Display.html?id=21530
    ...
    my ($csv, @fld);
    ok ($csv = Text::CSV_PP->new ({ quote_char => "~" }), "RT-18703: Fails to use quote_char of '~'");
