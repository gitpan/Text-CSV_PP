...
{   my @bad = (
    ...
    for (@bad) {
	my ($tst, $valid, $bad) = @$_;
	$csv = Text::CSV_PP->new ({ escape_char => "\\" });
...
{   my @bad = (
	...
	for (@bad) {
	    my ($tst, $ok, $bad) = @$_;
	    $csv = Text::CSV_PP->new ({ eol => $eol, binary => 1 });
...
#	[ 14, 0, qq{  1  ,  foo  ,  bar  ,  "baz"\t ,  quux  },	],
	...
	for (@bad) {
	    my ($tst, $ok, $bad) = @$_;
	    $csv = Text::CSV_PP->new ({ eol => $eol, binary => 1, escape_char => "+" });
	    ok ($csv,				"$s_eol / $tst - new - '$bad')");
	    is ($csv->parse ($bad), $ok,	"$s_eol / $tst - parse () fail");
...
foreach my $bin (0, 1) {
	...
	my $s_eol = _readable ($eol);
	my $csv = Text::CSV_PP->new ({ binary => $bin, eol => $eol });
	ok ($csv, "$s_eol - new)");
	my @bad = (
	    # test, line
	    [ 1, qq{"\r\r\n"\r},	],
